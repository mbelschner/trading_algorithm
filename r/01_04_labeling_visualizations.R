...
# - OHLC Candlestick Charts with Signals
#
# =============================================================================
...
# =============================================================================
# 6. OHLC CANDLESTICK CHART WITH SIGNALS AND META-LABELS
# =============================================================================

#' Plot OHLC candlestick chart with primary signals and meta-label outcomes
#'
#' Creates a professional candlestick chart showing:
#' - Classic OHLC candlesticks (green=bullish, red=bearish)
...
#'
#' @param dt data.table with OHLC data, primary_signal, meta_label
...
plot_price_with_signals <- function(
  ...
  required_cols <- c("datetime", "open", "high", "low", "close", "primary_signal", "meta_label")
  ...
  for (i in 1:nrow(top_weeks)) {
    ...
    # =========================================================================
    # CANDLESTICK CHART with tidyquant::geom_candlestick()
    # =========================================================================
    p <- ggplot(week_data, aes(x = datetime)) +
      geom_candlestick(
        aes(open = open, high = high, low = low, close = close),
        colour_up = "#26A69A",      # Green for bullish
        colour_down = "#EF5350",    # Red for bearish
        fill_up = "#26A69A",
        fill_down = "#EF5350",
        linewidth = 0.5
      )

    # Add EMA lines if available and requested
    if (show_ema) {
      if ("ema_fast" %in% names(week_data)) {
        p <- p + geom_line(aes(y = ema_fast), color = "#2196F3", linewidth = 0.8, alpha = 0.9)
      }
      if ("ema_slow" %in% names(week_data)) {
        p <- p + geom_line(aes(y = ema_slow), color = "#FF9800", linewidth = 0.8, alpha = 0.9)
      }
    }

    # Add TP/SL barrier lines if requested (vectorized with geom_segment)
    if (show_barriers && "tp_distance" %in% names(signals) && nrow(signals) > 0) {
      barrier_data <- signals[, .(
        ...
        tp_price = fifelse(primary_signal == 1, close + tp_distance, close - tp_distance),
        sl_price = fifelse(primary_signal == 1, close - sl_distance, close + sl_distance)
      )]

      # TP barriers (green dashed)
      p <- p + geom_segment(
        data = barrier_data,
        aes(x = datetime_start, xend = datetime_end, y = tp_price, yend = tp_price),
        color = "#4CAF50", linetype = "dashed", linewidth = 0.5, alpha = 0.7
      )

      # SL barriers (red dashed)
      p <- p + geom_segment(
        data = barrier_data,
        aes(x = datetime_start, xend = datetime_end, y = sl_price, yend = sl_price),
        color = "#F44336", linetype = "dashed", linewidth = 0.5, alpha = 0.7
      )
    }

    # Add signal markers
    if (nrow(signals) > 0) {
      # Long signals (upward triangles)
      long_signals <- signals[primary_signal == 1]
      if (nrow(long_signals) > 0) {
        p <- p + geom_point(
          data = long_signals,
          aes(x = datetime, y = arrow_y, color = outcome),
          shape = 24, size = 3.5, stroke = 1.5, fill = NA
        )
      }

      # Short signals (downward triangles)
      short_signals <- signals[primary_signal == -1]
      if (nrow(short_signals) > 0) {
        p <- p + geom_point(
          data = short_signals,
          aes(x = datetime, y = arrow_y, color = outcome),
          shape = 25, size = 3.5, stroke = 1.5, fill = NA
        )
      }
    }
    ...
    p <- p + scale_color_manual(
      values = c("TP Hit (Success)" = "#4CAF50",
                 "SL/Timeout (Failure)" = "#F44336",
                 "Unknown" = "#9E9E9E"),
      name = "Outcome"
    )
    ...
    p <- p + labs(
      title = sprintf("OHLC Candlestick Chart - Week %d", i),
      subtitle = sprintf("%s to %s | Signals: %d Long, %d Short | Outcomes: %d TP, %d SL",
                         week_start_str, week_end_str, n_long, n_short, n_tp, n_sl),
      x = NULL,
      y = "Price",
      caption = expression(
        paste(Delta, " Up = Long | ", Delta, " Down = Short | ",
              "Green = TP Hit | Red = SL/Timeout | ",
              "Blue Line = EMA Fast | Orange Line = EMA Slow"))
    ) +
      theme_tq() +
      theme(
        plot.title = element_text(face = "bold", size = 14),
    ...
    if (!is.null(output_path)) {
      filename <- sprintf("ohlc_signals_week_%d_%s.png", i, gsub("-", "", week_start_str))
  ...
  if (n_weeks > 1 && length(plots) > 1) {
    ...
    if (!is.null(output_path)) {
      ggsave(file.path(output_path, "ohlc_signals_combined.png"),
             combined, width = 16, height = 8 * n_weeks, dpi = 300)
      cat("Saved: ohlc_signals_combined.png\n")
    }
  }
...
# =============================================================================
# 6b. INTERACTIVE OHLC CHART (PLOTLY)
# =============================================================================

#' Create interactive OHLC candlestick chart with plotly
#'
#' @param dt data.table with OHLC and signal data
#' @param n_days Number of days to display
#' @param output_path Directory to save HTML file (optional)
#' @return plotly object
plot_ohlc_interactive <- function(dt, n_days = 5, output_path = NULL) {

  if (!requireNamespace("plotly", quietly = TRUE)) {
    warning("Package 'plotly' required for interactive charts. Install with: install.packages('plotly')")
    return(NULL)
  }
  ...
  max_date <- max(dt$datetime)
  min_date <- max_date - n_days * 24 * 60 * 60

  dt <- dt[datetime >= min_date]

  # Get signals
  signals <- dt[!is.na(primary_signal) & primary_signal != 0]

  # Build candlestick chart
  fig <- plotly::plot_ly(dt, x = ~datetime, type = "candlestick",
  ...
  if (nrow(signals) > 0) {
    # Long signals
    long_sig <- signals[primary_signal == 1]
    if (nrow(long_sig) > 0) {
      fig <- fig %>% plotly::add_markers(
        data = long_sig, x = ~datetime, y = ~low * 0.999,
        marker = list(symbol = "triangle-up", size = 12,
                      color = ifelse(long_sig$meta_label == 1, "#4CAF50", "#F44336")),
        name = "Long Signal", hoverinfo = "text",
        text = ~sprintf("Long @ %.2f<br>Outcome: %s", close,
                        ifelse(meta_label == 1, "TP Hit", "SL/Timeout"))
      )
    }

    # Short signals
    short_sig <- signals[primary_signal == -1]
    if (nrow(short_sig) > 0) {
      fig <- fig %>% plotly::add_markers(
        data = short_sig, x = ~datetime, y = ~high * 1.001,
        marker = list(symbol = "triangle-down", size = 12,
                      color = ifelse(short_sig$meta_label == 1, "#4CAF50", "#F44336")),
        name = "Short Signal", hoverinfo = "text",
        text = ~sprintf("Short @ %.2f<br>Outcome: %s", close,
                        ifelse(meta_label == 1, "TP Hit", "SL/Timeout"))
      )
    }
  }
  ...
  if ("ema_fast" %in% names(dt)) {
    fig <- fig %>% plotly::add_lines(x = ~datetime, y = ~ema_fast,
                                     line = list(color = "#2196F3", width = 1.5),
                                     name = "EMA Fast")
  }
  if ("ema_slow" %in% names(dt)) {
    fig <- fig %>% plotly::add_lines(x = ~datetime, y = ~ema_slow,
                                     line = list(color = "#FF9800", width = 1.5),
                                     name = "EMA Slow")
  }

  # Layout
  fig <- fig %>% plotly::layout(
    title = list(text = "Interactive OHLC Chart with Trading Signals"),
  ...
  if (!is.null(output_path)) {
    html_file <- file.path(output_path, "ohlc_interactive.html")
...
#' @param slippage_pips Slippage in pips
#' @param interactive Generate interactive plotly chart? (requires plotly)
generate_all_visualizations <- function(
  ...
  plot_price_with_signals(dt, n_weeks = 2, output_path,
                          show_barriers = TRUE, show_ema = TRUE, show_plot = FALSE)

  # Optional: Interactive chart
  if (interactive) {
    tryCatch({
      plot_ohlc_interactive(dt, n_days = 5, output_path)
    }, error = function(e) {
      cat("Note: Interactive chart skipped (plotly not available)\n")
    })
  }
...
# Module loaded message (optional)
# cat("\n=== LABELING VISUALIZATIONS MODULE LOADED ===\n")
# Available functions:
#   - plot_label_density_by_hour()
#   - plot_cumulative_edge()
#   - plot_barrier_profitability_heatmap()
#   - plot_sample_weight_distribution()
#   - plot_meta_label_performance()
#   - plot_price_with_signals()        [OHLC Candlesticks with tidyquant]
#   - plot_ohlc_interactive()          [Interactive plotly chart]
